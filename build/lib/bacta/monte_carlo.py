...
def bootstrap_ohlcv_series_multi(data: pd.DataFrame, n: int = 10, noise_std: float = 0.0, seed: int = None) -> List[pd.DataFrame]:
    """
    ...
    for _ in range(n):
        ...
        for symbol in symbols:
            ...
            bootstrapped_prices = np.exp(
                np.insert(bootstrapped_returns, 0, 0)).cumprod() * first_price

            # Rebuild OHLCV dataframe
            df_boot = df_symbol.copy()
            df_boot['close'] = bootstrapped_prices
            df_boot['open'] = df_boot['close'].shift(
                1).fillna(df_boot['close'])  # naive open
            df_boot['high'] = np.maximum(
                df_boot['open'], df_boot['close']) * (1 + 0.01)  # 1% range up
            df_boot['low'] = np.minimum(
                # 1% range down
                df_boot['open'], df_boot['close']) * (1 - 0.01)

            simulated.append(df_boot)

        df_simulated = pd.concat(simulated, keys=symbols, names=[
                                 'symbol', 'timestamp'])
