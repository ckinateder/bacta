...
import numpy as np
from util import load_sec_tickers, save_dataframe
...
def normalize_price_data_by_derived_data(
    price_data: pd.DataFrame, eps_small: pd.DataFrame
) -> pd.DataFrame:
    ...
    tickers = list(
        set(tickers) & set(eps_small.index.get_level_values("ticker").unique())
    )
        ...
        try:
            ...
            for date, row in ticker_prices.iterrows():
                ...
                if latest_eps != 0:
                    ...
                    normalized_prices.loc[(ticker, date), "close (e)"] = (
                        row["close"] / latest_eps
                    )
                    normalized_prices.loc[(ticker, date), "open (e)"] = (
                        row["open"] / latest_eps
                    )
                    normalized_prices.loc[(ticker, date), "high (e)"] = (
                        row["high"] / latest_eps
                    )
                    normalized_prices.loc[(ticker, date), "low (e)"] = (
                        row["low"] / latest_eps
                    )
                else:
                    # set to NaN
...
class SimFinClient:
    ...
    def get_derived_data(
        ...
        def get_derived_data_for_ticker(
            ticker: str, start_date: datetime = None, end_date: datetime = None
        ) -> pd.DataFrame:
            ...
            # make the request
            url = (
                f"https://backend.simfin.com/api/v3/companies/statements/compact?ticker={ticker}"
            ...
            derived_statement = next(
                (s for s in item["statements"] if s["statement"] == "DERIVED"), None
            )
        ...
        if isinstance(ticker, str):
            ...
            if condensed:
                ...
                ct.rename(
                    columns={"Earnings Per Share, Basic": "EPS TTM"}, inplace=True
                )
...
def plot_price_data(df: pd.DataFrame, title: str = None):
    ...
    plt.xticks(
        series.index[:: len(series) // 20],
        series.index[:: len(series) // 20].strftime("%Y-%m-%d"),
        rotation=45,
    )
    ax2 = plt.twinx()
...
if __name__ == "__main__":
    ...
    # save_dataframe(bars, 'utility_bars.csv')
    ...
    # save_dataframe(derived_data, 'derived_data.csv')
    derived_data = pd.read_pickle("data/derived_data.pkl")

    derived_data = derived_data[
        ["Fiscal Period", "Fiscal Year", "Earnings Per Share, Basic"]
    ]
    ...
    # save_dataframe(normalized_data, 'normalized_data.csv')
